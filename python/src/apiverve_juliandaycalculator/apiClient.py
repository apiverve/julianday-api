...
class JuliandayAPIClientError(Exception):
...
class ValidationError(JuliandayAPIClientError):
...
class JuliandayAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"date": {"type": "string", "required": True, "format": "date"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the JuliandayAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises JuliandayAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/julianday'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[JuliandayAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns JulianDayCalculatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises JuliandayAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise JuliandayAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise JuliandayAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise JuliandayAPIClientError(f"Request failed: {str(e)}")
